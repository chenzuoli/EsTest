...
public class DeleteEsByDate {
    private static Logger logger = Logger.getLogger(DeleteEsByDate.class);
    private static TransportClient client;

    public static void main(String[] args) {
        if (args.length != 8) {
            String returnMsg = "please enter the right args number!";
        ...
        if ("1".equals(args[7])) {
            ...
            logger.error("please enter correct delete mode: 1 is delete equals, 2 is delete not equals!");
        }
    }
    ...
    private static void deleteDocumentByQuery(String index, String type, String col, String val) {
        ...
        builder.source().setIndices(index)
                .setTypes(type)
                .setQuery(QueryBuilders.matchQuery(col, Integer.parseInt(val)));
        BulkByScrollResponse response = builder.get();
        long deleted = response.getDeleted();
        logger.info("delete number of document is " + deleted);
    }
    ...
    private static void deleteOthDocByQuery(String index, String type, String col, String val) {
        ...
        boolQuery.mustNot(QueryBuilders.matchQuery(col, val));
        builder.source().setIndices(index)
                .setTypes(type)
                .setQuery(boolQuery);
